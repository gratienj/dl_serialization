...
class TRTEngineTester
{
public:
    TRTEngineTester(std::string const& onnx_model_path, int batch_size)
    ...
    //!
    bool build(int nb_vertices,
        int nb_edges);

    //!
    //! \brief Runs the TensorRT inference engine for this sample
    //!
    bool infer(std::vector<float> const& c0,
...
private:
    ...
    template<typename ValueT>
    bool _infer(std::vector<ValueT> const& c0,
