...
//!
bool TRTEngineTester::build(int nb_vertices, int nb_edges)
{
std::cout<<"CREATE BUILDER"<<std::endl ;
    ...
    std::cout<<"SET DIMS : "<<input0->getName()<<" "<<dims0.nbDims <<" "<<nb_vertices<<" "<<nb_edges<<std::endl ;
    if(dims0.nbDims == 2)
    {
      profile->setDimensions(input0->getName(), OptProfileSelector::kMIN, Dims2{1, nb_vertices});
      profile->setDimensions(input0->getName(), OptProfileSelector::kOPT, Dims2{1, nb_vertices});
      profile->setDimensions(input0->getName(), OptProfileSelector::kMAX, Dims2{1, nb_vertices});
    }

    std::cout<<"SET DIMS : "<<input1->getName()<<" "<<dims1.nbDims<<" "<<nb_vertices<<" "<<nb_edges<<std::endl ;
    if(dims1.nbDims == 2)
    {
      profile->setDimensions(input1->getName(), OptProfileSelector::kMIN, Dims2{1, nb_vertices});
      profile->setDimensions(input1->getName(), OptProfileSelector::kOPT, Dims2{1, nb_vertices});
      profile->setDimensions(input1->getName(), OptProfileSelector::kMAX, Dims2{1, nb_vertices});
    }

    std::cout<<"SET DIMS : "<<input2->getName()<<" "<<dims2.nbDims<<" "<<nb_vertices<<" "<<nb_edges<<std::endl ;
    if(dims2.nbDims == 2)
    {
      profile->setDimensions(input2->getName(), OptProfileSelector::kMIN, Dims2{1, nb_edges});
      profile->setDimensions(input2->getName(), OptProfileSelector::kOPT, Dims2{1, nb_edges});
      profile->setDimensions(input2->getName(), OptProfileSelector::kMAX, Dims2{1, nb_edges});
    }

    std::cout<<"SET DIMS : "<<input3->getName()<<" "<<dims3.nbDims<<" "<<nb_vertices<<" "<<nb_edges<<std::endl ;
    if(dims3.nbDims == 2)
    {
      profile->setDimensions(input3->getName(), OptProfileSelector::kMIN, Dims2{1, nb_edges});
      profile->setDimensions(input3->getName(), OptProfileSelector::kOPT, Dims2{1, nb_edges});
      profile->setDimensions(input3->getName(), OptProfileSelector::kMAX, Dims2{1, nb_edges});
    }

    std::cout<<"SET DIMS : "<<output->getName()<<" "<<out_dims0.nbDims<<" "<<nb_vertices<<" "<<nb_edges<<std::endl ;
    /*
    if(out_dims0.nbDims == 1)
    {
      profile->setDimensions(output->getName(), OptProfileSelector::kMIN, Dims{1});
      profile->setDimensions(output->getName(), OptProfileSelector::kOPT, Dims{1});
      profile->setDimensions(output->getName(), OptProfileSelector::kMAX, Dims{1});
    }*/
...
//!
bool TRTEngineTester::constructNetwork(SampleUniquePtr<nvinfer1::IBuilder>& builder,
...
bool TRTEngineTester::_infer(std::vector<ValueT> const& c0,
                            std::vector<ValueT> const& c1,
                            std::vector<int> const& edge_to,
                            std::vector<int> const& edge_from,
                            ...
                            int nb_edges)
{
    ...
    std::cout<<"PROCESS INPUT : "<<c0.size()<<" "<<dims0<<std::endl ;
    if(dims0.nbDims == 2)
    {
      Dims2 inputDims{1,nb_vertices} ;
      mInput0.hostBuffer.resize(inputDims) ;
      if (!processInput(c0,mInput0))
      {
          return false;
      }
    ...
    std::cout<<"PROCESS INPUT : "<<c1.size()<<" "<<dims1<<std::endl ;
    if(dims1.nbDims == 2)
    {
      Dims2 inputDims{1,nb_vertices} ;
      mInput1.hostBuffer.resize(inputDims) ;
      if (!processInput(c1,mInput1))
      {
          return false;
      }
    ...
    std::cout<<"PROCESS INPUT : "<<edge_to.size()<<" "<<dims2<<std::endl ;
    if(dims2.nbDims == 2)
    {
      Dims2 inputDims{1, nb_edges} ;
      mInput2.hostBuffer.resize(inputDims) ;
      if (!processInput(edge_to,mInput2))
      {
          return false;
      }
    ...
    std::cout<<"PROCESS INPUT : "<<edge_from.size()<<" "<<dims3<<std::endl ;
    if(dims3.nbDims == 2)
    {
      Dims2 inputDims{1, nb_edges} ;
      mInput2.hostBuffer.resize(inputDims) ;
      if (!processInput(edge_from,mInput3))
      {
          return false;
      }
    ...
    if(outdims.nbDims == 2)
    {
      Dims2 outputDims{1, nb_vertices} ;
...
bool TRTEngineTester::infer(std::vector<float> const& c0,
                            std::vector<float> const& c1,
                            std::vector<int> const& edge_to,
                            std::vector<int> const& edge_from,
                            ...
                            int nb_edges)
{
  return _infer(c0,c1,edge_to,edge_from,outputs,nb_vertices,nb_edges) ;
}
...
//!
template<typename ValueT>
bool TRTEngineTester::_processInput(std::vector<ValueT> const& inputs,
...
bool TRTEngineTester::processInput(std::vector<float> const& inputs,
                             samplesCommon::ManagedBuffer& mInput)
{
...
bool TRTEngineTester::processInput(std::vector<int> const& inputs,
                             samplesCommon::ManagedBuffer& mInput)
{
  return _processInput(inputs, mInput) ;
}
